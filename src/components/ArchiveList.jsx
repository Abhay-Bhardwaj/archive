...
export default function ArchiveList({searchTerm, users}) {
  ...
  return (
    <div className='w-full h-full bg-slate-100 p-2 rounded-md'>
        ...
        <hr/>
        {(loading)? <Loader2 className="animate-spin w-20 h-20 " />
        ...
        <div className='flex flex-row justify-center'>
          <button onClick={() => setPagination(pagination - 1)} disabled={pagination === 1 || pagnatedData.length===0} className='bg-blue-500 disabled:bg-blue-200 p-2 rounded-md '><ArrowLeft/></button>
          <span className='p-2'>{pagination}</span>
          <button onClick={() => setPagination(pagination + 1)} disabled={pagination === Math.ceil(finalData.length / perPageArticle || pagnatedData.length===0)} className='bg-blue-500 disabled:bg-blue-200 p-2 rounded-md'><ArrowRight/></button>
